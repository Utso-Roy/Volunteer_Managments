...
const ManagePost = () => {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    html: `
      ...
      <input id="volunteersNeeded" class="swal2-input" type="number" placeholder="Volunteers Needed" value="${post.volunteersNeeded}">
      <input id="deadline" class="swal2-input" type="date" value="${new Date(post.deadline).toISOString().split('T')[0]}">
    `,
    focusConfirm: false,
    showCancelButton: true,
    preConfirm: () => {
     ...
     <h2 className="text-2xl font-semibold mb-6 text-[#014f86] border-l-4 border-[#0267af] pl-3">
           All Volunteer Post
              ...
              <div className="grid sm:grid-cols-2 gap-4 md:grid-cols-3">
                  ...
                  <div key={singleData._id} className="card bg-base-100 p-4 shadow-sm">
  <figure>
    <img
  ...
  <div className="card-body">
                                  ...
                                  <p> Category : <strong>{singleData.category }</strong></p>
                                  <p>volunteersNeeded:{ singleData.volunteersNeeded}</p>
                               
                                  <div className="card-actions justify-end">
                                            <button onClick={()=>handlePostUpdate(singleData)} className="bg-[#0267af] cursor-pointer text-white px-3 py-1.5 rounded hover:bg-[#0267afb6]">  <FaEdit className="inline mr-1" />Update</button>
      <button onClick={()=>handleAddPostDelete(singleData._id)} className="bg-red-500 cursor-pointer text-white px-3 py-1.5 rounded hover:bg-red-600"> <FaTrash className="inline mr-1" />Delete</button>
